...
import { HttpClient, HttpHeaders } from '@angular/common/http'; // Se importa el HttpClient para poder realizar peticiones http
...
export class CustomerService {
  ...
  getCustomerById(id: string){ // Se define el método getCustomerById con el parámetro id de tipo string
    return this.http.get<Customer>(`${API_URL}/id/${id}`, generateHeaders()); // El parámetro id se concatena a la url de la api para obtener el customer por su id al hacer el llamado a la api.
  }

  // El siguiente método se utiliza para obtener todos los customers
  getAllCustomers() {
    return this.http.get<ApiResponse>(API_URL, generateHeaders()); // Se hace el llamado a la api para obtener todos los customers
  }
  ...
  postCustomer(body: Customer){ // Se define el método postCustomer con el parámetro body de tipo Customer
    return this.http.post<string>(API_URL, body, generateHeaders()); // Se hace el llamado a la api para crear un customer
  }
  ...
  deleteCustomer(id: string){ // Se define el método deleteCustomer con el parámetro id de tipo string
    return this.http.delete<string>(`${API_URL}/${id}`,generateHeaders()); // El parámetro id se concatena a la url de la api para eliminar el customer por su id al hacer el llamado a la api.
  }
  ...
  putCustomer(id: string, body: Customer){ // Se define el método putCustomer con los parámetros id de tipo string y body de tipo Customer
    return this.http.put<string>(`${API_URL}/${id}`, body, generateHeaders()); // El parámetro id se concatena a la url de la api para actualizar el customer por su id y también se pasa la información a actualizar.
  }
}
